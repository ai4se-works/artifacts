...
\title{From Pilot Evidence to Conditional Scale: An AI4SE Operating Stack for Mid-to-Large Organizations%
...
We present an AI4SE operating stack for that setting---a layered reading of
classical software engineering together with Lean/Agile team modes whose
primary value object shifts from stories toward specification-driven
\emph{change requests}---and an Explore, Consolidation, and conditional Scale
adoption rhythm in which stage closeout and admission are evidence reviews
...
management met Explore closeout and opened Consolidation, but did not
admit Scale.
...
the same stack and rhythm after co-located alternatives were judged
infeasible and is now in a mid-July to mid-September 2026 Explore that
remains open.
The paper contributes the operating stack and the Org-A stage-and-asset
record; Org-B supplies only a decision-fact vignette, not a second completed
case.
...
\begin{IEEEkeywords}
AI for software engineering, AI4SE operating stack, industrial experience report
\end{IEEEkeywords}
...
We present an AI4SE operating stack---and an Explore--Consolidation--Scale
adoption rhythm---for advancing industrial AI4SE without treating calendar
completion as admission---validated primarily by Org-A's recorded Explore
closeout and Scale-admission dispositions and asset ledger, not by a method
brochure alone.
...
Explore still open).
The transferable object is the stack-plus-stage-criteria contract---not a tool catalog
...
(goals, deliverables, and closeout, admission, and exit tests)?
\item \textbf{Q2.}
...
are those Org-A management actually decided (Explore closeout met / Scale admission held) and the
...
\textbf{Contributions.}
(1)~an AI4SE operating stack (Pressman-layer upgrade; Lean/Agile$\rightarrow$team
paradigm$\rightarrow$SDD with story$\rightarrow$change-request value objects) plus an
Explore--Consolidation--Scale adoption rhythm with closeout/admission criteria
(citable / runnable / transferable readiness; stub~$\neq$~ready);
...
(3)~a bounded Org-B decision fact only: the same stack and rhythm were selected after
alternative approaches were judged infeasible, under an IPD constraint, with
Explore still open (not a second completed stage-criteria packet).

Bounded Explore-phase effectiveness and maturity signals appear here only as
stage-criteria inputs with stated limits.
...
scheme, not the operating stack and adoption rhythm.  No Org-A workshop or maturity numbers from
that manuscript are imported here.}

Section~\ref{sec:background} supplies the conceptual overlay.
Section~\ref{sec:path} defines the stages and closeout/admission criteria.
...
The operating stack and adoption rhythm are the contribution of this paper.
This section supplies a compact vocabulary for \emph{why adoption is staged}
and \emph{what the stack carries}.
...
empirical validation of our stage criteria; denser positioning appears in
...
\subsection{Operating Stack: Value Layer, Team Paradigm, and SDD}
\label{sec:operating-stack}

Pressman's layers answer \emph{what} to upgrade; delivery teams still need an
\emph{operating stack} that says how Lean/Agile work continues under AI4SE.
Figure~\ref{fig:operating-stack} states three nested layers we used with
sponsors and coaches.
The top layer keeps Lean/Agile value principles (flow, pull, feedback, respect
for people).
The middle layer keeps classical team operating modes---Scrum, Kanban, or
Scrumban---as the cadence and role container (Org-A primarily Scrum; Org-B
combines IPD with multiple team modes).
The bottom layer is Spec-Driven Development (SDD): Align, Verify, and Merge
reviews that protect each change under the chosen paradigm \(p\).

The industrial claim is deliberately narrow: those team modes remain
effective; what shifts is the primary \emph{unit of value flow}.
Where boards and backlogs once centered on user stories, SDD centers on a
citable \emph{change request} (and its Align$\rightarrow$Verify$\rightarrow$Merge
contract) as the object that carries definition and verification evidence.
Stories and other backlog items may still exist; they no longer substitute for
an inspectable change contract when agents draft and apply increments.
We reserve the word \emph{gate} for those process-level Align/Verify/Merge
reviews in the method pack, and we use \emph{closeout} / \emph{admission} for
organizational stage decisions in Section~\ref{sec:path}---so the two loops
do not share G1/G2 numbering.
...
\begin{tikzpicture}[
  ...
  >=Latex,
  layer/.style={
    draw, rounded corners=1.5pt, align=left, inner sep=3pt,
    text width=0.92\columnwidth, minimum height=9mm
  }
]
\node[layer] (val) {%
  \textbf{Value layer (Lean/Agile)}\\
  flow $\cdot$ pull $\cdot$ feedback $\cdot$ respect for people};
\node[layer, below=2mm of val] (team) {%
  \textbf{Team operating paradigm}\\
  Scrum $\mid$ Kanban $\mid$ Scrumban
  (cadence, roles, inspect--adapt)};
\node[layer, below=2mm of team] (sdd) {%
  \textbf{SDD method layer}\\
  Change request $+$ Align / Verify / Merge
  (definition and verification contract)};
\draw[->,thick] (val) -- (team);
\draw[->,thick] (team) -- (sdd);
\node[
  draw, dashed, rounded corners=1pt, align=center, inner sep=2.5pt,
  text width=0.92\columnwidth, below=2.5mm of sdd
] (shift) {%
  Value object shift: user story $\longrightarrow$ SDD change request\\
  \emph{same inspect--adapt loop; different work-item contract}};
\end{tikzpicture}
\caption{Three-layer AI4SE operating stack.
Classical Lean/Agile team modes remain; the primary value-flow object moves
from stories toward SDD change requests.}
\label{fig:operating-stack}
\end{figure}
...
substitute for stage closeout and admission.
...
the operating stack and adoption rhythm.
...
\emph{verification leadership} (acceptance and human sign-off).
...
Both Effectiveness and Harmony are inputs to Explore closeout and Scale admission.
...
harness privileges fast prompt--edit loops and lighter formal Align/Verify/Merge checks.
Neither is universally ``correct''; the stage packet must record which \(p\)
...
The conceptual layer therefore explains \emph{why} stage criteria ask for method,
asset, ownership, and transfer evidence; it does not add a parallel
transformation method.

\section{Adoption Rhythm: Explore to Conditional Scale}
\label{sec:path}
% EVIDENCE: ../00-shared-evidence/scale-path.md, phase1-gate.md

\subsection{Why Closeout and Admission, Not a Calendar, Advance the Path}
...
verification time---another reason reviews must examine evidence rather than
calendars.

The transformation path nests two control loops.
\emph{Stage closeout and admission decisions} decide whether the organization may enter
Consolidation or admit a Scale wave.
Inside delivery work, shorter \emph{Align/Verify/Merge} reviews---align a delta
specification, verify a candidate increment, then merge and
archive---protect each change under the chosen paradigm \(p\).
Stage criteria ask whether method, assets, and ownership are transferable;
Align/Verify/Merge reviews ask whether a particular change is definition-complete and
evidence-backed.
Confusing the two is a common failure: a team that passes local change
reviews can still fail Scale admission if receiving teams cannot inherit the method.

% FACT-III-A
The path instantiated at Org-A comprises \emph{Explore},
\emph{Consolidation}, and \emph{Scale} (Fig.~\ref{fig:stage-path}).
...
\node[decide,right=5mm of explore] (ec) {EC};
\node[stage,right=5mm of ec] (consolidate) {
...
\node[decide,right=5mm of consolidate] (sa) {SA};
\node[stage,right=5mm of sa] (scale) {
  \textbf{Stage 3: Scale}\\
  Diffuse with quality-consistent guardrails\\
  \emph{Outputs:} rollout handbook, new-team validation\\
  \emph{Exit:} new team self-runs
};
\draw[flow] (explore) -- (ec);
\draw[flow] (ec) -- (consolidate);
\draw[flow] (consolidate) -- (sa);
\draw[flow] (sa) -- (scale);
\node[below=2.5mm of ec,align=center,font=\scriptsize] {Explore closeout};
\node[below=7mm of ec,align=center,font=\scriptsize] {assets + bounded signals};
\node[below=2.5mm of sa,align=center,font=\scriptsize] {Scale admission};
\node[below=7mm of sa,align=center,font=\scriptsize] {method + owners};
\end{tikzpicture}
\caption{Explore--Consolidation--Scale adoption rhythm with Explore closeout
(EC) and Scale admission (SA) criteria.  The figure specifies evidence
required to advance; it does not indicate that Org-A completed Scale.}
\label{fig:stage-path}
\end{figure*}
...
prompt a Scale-admission review, but unmet criteria block admission.
...
\subsection{Closeout and Admission Semantics}

% FACT-III-E
Explore closeout is a \emph{method--asset closeout}.  Management asks whether a playbook can be
...
Consolidation-to-Scale path is concrete enough to discuss.  A positive Explore closeout
therefore licenses method hardening, not broad rollout.  Table
\ref{tab:stage-criteria} makes this decision contract explicit.

Scale admission is stricter because it concerns transfer beyond the pilot setting.  It
...
verification authority remains human-held under the nested Align/Verify/Merge reviews.
...
Consolidation.
This interpretation makes a stage decision a learning control, rather than a ceremonial
approval checkpoint.

\subsection{Operating Stage Criteria as an Evidence Review}

A usable stage review needs a review packet, a decision owner, and a recorded
...
perfection in the current one.  At Explore closeout, an incomplete executable scaffold can
be acceptable when it is disclosed and when the Consolidation plan names the
hardening work.  At Scale admission, the same scaffold is a blocker because a receiving
...
A stage disposition has three possible forms.  \emph{Advance} means the exit
...
The review also separates two kinds of uncertainty.  Product uncertainty asks
...
separately explains why a positive Explore closeout and a negative Scale admission can be the expected,
...
supports ``can run once'' and justified meeting Explore closeout; it does not establish
repeatability across teams.

% FACT-III-G
\textbf{Layer 3---Scale remains conditional.}
Org-A's pilot succeeded at Explore, but Scale admission was not met at closeout.
...
\subsection{Anti-Patterns Exposed by the Criteria}
...
repeatable.  Explore closeout exposes this weakness by requiring a citable method and
bounded evidence; Scale admission exposes it by requiring transfer and ownership.
...
cross-team replicability.  Separating Explore closeout from Scale admission preserves the useful result
...
a change also declare the change verified.  Nested Align/Verify/Merge reviews and the
Harmony verification function exist to forbid that shortcut: drafts and
execution may be agentic; sign-off remains human.
...
allowed to relax the client's existing quality controls.
...
case record comprises the phase timeline, the stage-criteria definition,
...
\node[event,right=4mm of kickoff] (w1) {\textbf{W1}\\early Jul.\\engineering base};
\node[event,right=4mm of w1] (w2) {\textbf{W2}\\mid-Jul.\\entry and metrics};
\node[event,right=4mm of w2] (w3) {\textbf{W3}\\mid/late Jul.\\review and test};
\node[event,right=4mm of w3] (w4) {\textbf{W4}\\late Jul.\\playbook + validation};
\node[event,right=4mm of w4] (close) {\textbf{Closeout}\\early Aug.\\EC met};
...
\draw[flow] (w4) -- (close);
\end{tikzpicture}
\caption{Org-A Explore timeline (week-level windows).  The early-August
review met Explore closeout and opened Consolidation; it did not meet
Scale admission.}
...
exercise, and opportunity-theme plan.  Workshop W1 (early July) addressed
engineering foundations: harness/context, knowledge, and brownfield
specification-driven development.  W2 (mid-July) covered metrics and
requirements entry and included a midterm management showcase.  W3
(mid-to-late July) addressed review, agent validation, and test engineering.
W4 (late July) ran two tracks in parallel: playbook v0.5 authoring and
specification-based validation in the delivery track.  An early-August
executive readout closed Explore, aligned the Consolidation path, and
explicitly did not claim Scale admission.
...
\subsection{From Activities to Stage Evidence}
...
series of facilitated sessions into inspectable inputs to Explore closeout.
...
team a stable object to revise and later cite.  The closeout review then assessed the
...
playbook could satisfy its Explore-closeout criterion while the largely stubbed L3 pack
could not satisfy Scale admission.
...
Version 1.0 was frozen, citable, and sufficient for the Explore closeout
(public method card released; full handbook withheld). &
...
increments---well enough to be shown and referenced at closeout.
...
A public method card distilled for inspection (chapter map, five first
principles, Living-Spec artifact chain, 5+1 stages with Align/Verify/Merge
Align/Verify/Merge reviews, paraphrased normative clauses, and declared v1 gaps) is released with
the data pack;\footnote{See
\texttt{l1-method-extract.md} under the Data Availability URL.}
the full client-facing handbook text is not.
...
for early playbooks.
Non-ROI intensity at closeout was closeout-visible without claiming return:
four workshop weeks plus kickoff/readout, twelve themes (ten released),
$11{+}11$ L2 assets, a largely stubbed L3 main chain, and Explore closeout met / Scale admission held.
...
application, and human-owned verification under retained quality controls.
...
paper, L4 establishes that evidence boundaries were available to the closeout review;
it is not used to introduce the companion paper's quantitative findings.

\subsection{Phase-One Explore Closeout Decision}

% FACT-IV-H
At the early-August executive readout, management applied five questions: whether
...
be discussed concretely.  The result was a pass for the Explore closeout.  The
...
verdict.  Explore closeout answered whether the organization had enough evidence to harden
a feasible method; Scale admission would answer whether that hardened method could be
...
its planned activities.  The stage-path account preserves the observed
...
the feasibility argument was in place because stages and closeout/admission criteria had been
defined.  Layer 2 was observed because Org-A completed an end-to-end Explore
cycle and met Explore closeout.  Layer 3 remained conditional because Scale admission had not been
...
assessment can apply Scale admission to those same declared conditions without changing
...
structure.
We do not report a clause-level mapping of Explore closeout or Scale admission onto Org-B's IPD
...
for its setting and selected the operating stack and adoption rhythm described here as the
...
scale has started.
Concrete negative process evidence at freeze: closeout/admission host reviews inside IPD
...
admission withheld, with an in-repo ledger and stage packet.
...
\begin{table}[t]
\caption{Org-A and Org-B on the same stack and rhythm}
...
Process & Multi-team agile; retained quality controls & IPD incumbent; path must adapt \\
Pilot shape & Enablement plus validation vehicle & Multi-mode team sample in one wave \\
Adoption & Sponsored enablement; Explore closeout met / Scale admission held & Method switch after infeasible alternatives \\
...
Evidence depth & In-repo ledger and stage packet & Author briefing; no intranet mirror \\
\bottomrule
\end{tabularx}
\end{table}
...
which IPD reviews will host Explore closeout or Scale admission at Org-B, and it does not invent
mid-September Explore closeout status.

Classical product Stage-Gate practice already separates phases with business-case
...
publishes a withheld Scale decision beside a passed Explore closeout, and
...
Put Explore closeout and Scale admission on separate agenda lines.
...
and ship a starter kit another team can run; keep unmet Scale-admission items on a public
hold list.
\item \textbf{Keep negative evidence visible.}
An unmet Scale admission, an in-progress Explore, and unaccepted deliverables are
...
qualitative stage-criteria inputs (Org-A L4; Org-B Goal~5 as scheme design).
...
operating stack and adoption rhythm.
...
second site with a matching stage-criteria packet.
Mode diversity inside Org-B improves within-organization coverage; it does
not create a multi-enterprise sample.

\textbf{Construct validity.}
``Scale-ready'' is a criteria-and-asset construct in this paper, not a
...
Authors facilitated enablement, stage-criteria framing, and asset authoring at
...
publishing negative evidence (stubbed L3; unmet Scale admission; Org-B still in
Explore), and by recording stage dispositions as client management decisions
...
A positive Explore closeout plus a negative Scale admission is an internally consistent pair, not a
contradictory evaluation.
Treating either decision as a completed-scale result, or treating Org-B's
...
operating stack and adoption rhythm as a designed enablement artifact~\cite{Hevner2004DSR}
...
consensus as multi-site validation of our stage criteria, and do not claim the
...
Our unit of analysis is a AI4SE operating stack with an adoption rhythm and the
closeout and admission criteria of its stages, not a new maturity model.

\textbf{Differentiation from generic Stage-Gate vocabulary.}
...
Our rhythm reuses the \emph{idea} of evidence-based admission without adopting Stage-Gate vocabulary for the organizational layer---and the review
...
Generic Stage-Gate vocabulary does not, by itself, prevent equating a coding-assistant
...
The transferable artifact is the operating stack and adoption rhythm: Explore, Consolidation,
and conditional Scale, operated as evidence reviews at Explore closeout and Scale admission.
...
Explore under IPD; it does not contribute a second completed stage-criteria packet.
...
They should not convert an unmet Scale admission, or an in-progress Explore, into a
completed-scale claim.

\section*{Acknowledgment}
We thank Org-A and Org-B engineering participants for stage reviews and
...
Anonymized, self-contained evidence for this manuscript is released at
\url{https://github.com/ai4se-works/artifacts/tree/main/manuscripts/stage-gated-ai4se-path/data}
(CC-BY-4.0).
The pack includes a stage-criteria rubric (Explore closeout / Scale admission), a public L1 method card (chapter map,
principles, artifact chain, process Align/Verify/Merge reviews, paraphrased normative clauses, and
declared v1 gaps), an L1--L4 asset ledger, a twelve-theme index with
observation flags, a redacted L3 stub map, and the Org-A timeline used in the
case narrative.
It is intended for inspection of those claims without access to partner
intranets.
The full client-facing playbook text, opportunity Spec/BP bodies, plugin
sources, unredacted change records, and Org-B workspace content remain
confidential.
Quantitative maturity and workshop tables are not reproduced here; they appear
in the companion measurement data pack in the same artifacts repository.
